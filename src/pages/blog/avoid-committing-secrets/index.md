---
title: Avoid Committing Secrets to Source Control using KeyVault
date: "2018-11-03T11:35:03.284Z"
excerpt: "Learn how to avoid commiting secrets to source control using KeyVault and User Secrets Manager. Step by step process using PowerShell to manage secrets securely with a team."
---
Keeping secrets is hard. I remember one time my friend confided in me a secret about himself and I had such a hard time keeping that secret from the world. Luckily for keeping secrets in our code, Microsoft and Azure has made that a lot easlier. In the following post we will explore how to manage our secrets both locally and in production. Over the last few months I have been creating a React App with C#. And a huge problem with my project has been how to share the code publicly without leaking my keys in a public github repository. 

But also how to allow other users to get started contributing or even clone my repo and start on their own project, because so much of the project has magic configuration setting managed in Azure. As more projects move to the cloud, we will run into this problem more and more. How to share infrastructure with the public? Well you can't really because it all costs money and is tied to a subscription.

## Azure Resource Manager Templates to the rescue
So what is the next best thing? Well, I've decided to lower the barrier of entry, by scripting the infrastructure via PowerShell and Azures Resource Manager templates. Using Powershell we can leverage infrastructure as code and automate deployment, script configuration, and manage secrets. You can see the [source code on github](https://github.com/bharney/JSStarter). But the starting point for trying to setup the project locally means creating Azure Resources like a SQL Database, KeyVault, Blob Storage, etc... Luckily the AzureResourceGroup project handles the ARM template and configuration to manage the environment locally and during deployment. This allows stranger to script their environment to match the project structure, and ultimately avoiding having to commit secrets to source control.
